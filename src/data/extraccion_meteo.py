...
class FeatureBuilder:
    ...
    def compute_national_temperature(
        ...
        Para cada HORA calcula T_nac = Σ(T_ciudad_i_hora × peso_i)
        donde peso_i = población_i / Σ población.

        Granularidad horaria para alinearse con la demanda REE (indicador 460)
        y permitir el join directo por datetime en build_dataset.py.
        """
        if hourly_df.empty or pop_df.empty:
        ...
        # Unir pesos a datos horarios
        merged = hourly_df.merge(pop_latest[["ciudad", "peso"]], on="ciudad", how="left")
        merged["temp_pond"]      = merged["temperatura"]      * merged["peso"]
        merged["sensacion_pond"] = merged["sensacion_termica"] * merged["peso"]

        # Agregar por datetime → temperatura nacional horaria
        national = (
            merged
        ...
        # HDD y CDD horarios (base 18 °C)
        # Permiten detectar a qué hora exacta arranca el consumo de AC/calefacción
        national["HDD"] = np.maximum(0.0, COMFORT_TEMP - national["t_nac"])
        national["CDD"] = np.maximum(0.0, national["t_nac"] - COMFORT_TEMP)

        # Columnas temporales derivadas (útiles para feature engineering posterior)
        national["datetime"] = pd.to_datetime(national["datetime"])
        national["fecha"]    = national["datetime"].dt.date
        national["hora"]     = national["datetime"].dt.strftime("%H:%M")
        national["hora_num"] = national["datetime"].dt.hour
        national["año"]      = national["datetime"].dt.year
        national["mes"]      = national["datetime"].dt.month

        national["fuente"] = "Open-Meteo (ERA5) + INE Padrón"

        return national[[
            "datetime", "fecha", "hora", "hora_num", "año", "mes",
            "t_nac", "sensacion_nac", "HDD", "CDD",
            "ciudades_usadas", "fuente",
        ]]
...
class Extractor:
    ...
    def run(self) -> Dict[str, pd.DataFrame]:
        ...
        log.info("\n[3/4] Temperatura nacional ponderada por población (horaria)…")
        t_nac_df = self.feats.compute_national_temperature(hourly_df, pop_df)
